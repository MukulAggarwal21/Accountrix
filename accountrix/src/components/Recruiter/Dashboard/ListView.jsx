...
//     const date = interview.time.split(' ')[0]; // Extract date part
//     if (!groups[date]) {
//       groups[date] = [];
//     }
...
//                               <span className={`ml-2 text-xs px-2 py-1 rounded-full ${getStatusColor(interview.status)}`}>
//                                 {interview.status}
//                               </span>
//                             </div>
...
const ListView = ({ interviews, onClickInterview, onDeleteInterview }) => {
  ...
  // Group interviews by date
  const groupedInterviews = interviews.reduce((groups, interview) => {
    const date = interview.date.toLocaleDateString('en-US', {
      weekday: 'long',
      month: 'short',
      day: 'numeric',
    });
